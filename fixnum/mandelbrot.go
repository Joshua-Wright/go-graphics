...
func (m *MandelbrotPerPixel) GetPixel(i, j int64) {
	if m.primaryDelegate == nil {
		print("don't use this MandelbrotPerPixel directly!")
...
func NewMandelbrotPerPixel(width, height, maxIter int64,
	...
	dr := FromBigFloat(fourBf)

	centerR_, err := FromString(centerR)
	if err != nil {
		panic(err)
	}
	centerI_, err := FromString(centerI)
	if err != nil {
		panic(err)
	}
	threshold2, err := FromString(threshold)
	if err != nil {
		panic(err)
	}
