...
func main() {
	...
	out, err := memory_mapped.CreatePFMGray(int64(width), int64(height), g.ExecutableNameWithExtension("pfm"))
	g.Die(err)
	...
	out2, err := memory_mapped.OpenPFMGray(g.ExecutableNameWithExtension("pfm"))
	g.Die(err)
	defer out2.Close()
